...
export default function BrainsPage() {
  ...
  return (
    <div className="min-h-screen p-6">
      ...
      {/* Create Modal */}
      {showCreateModal && <Modal title={t.modalCreate} onClose={() => { setShowCreateModal(false); setFormData({ name: "", description: "", icon: "" }); }}>
      ...
      {/* Edit Modal */}
      {showEditModal && selectedBrain && <Modal title={t.modalEdit} onClose={() => { setShowEditModal(false); setSelectedBrain(null); setFormData({ name: "", description: "", icon: "" }); }}>
        <ModalForm formData={formData} setFormData={setFormData} />
        <ModalActions onCancel={() => { setShowEditModal(false); setSelectedBrain(null); setFormData({ name: "", description: "", icon: "" }); }} onConfirm={handleEdit} confirmText={t.btnSave} />
      </Modal>}
...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-[var(--bg-card)] dark:bg-[var(--bg-card)] rounded-xl w-full max-w-md dao-card" onClick={(e) => e.stopPropagation()}>
